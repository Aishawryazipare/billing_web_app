...
class SalesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($search != "") {
            ...
            //     ->toSql();
            $q = \App\Item::where(function ($fq) use ($cid, $search) {

                $fq->where('is_active', '=', 0);
                $fq->where('cid', '=', $cid);
                $fq->where(function ($fq) use ($search) {

                    $fq->where('item_name', 'LIKE', '%' . $search . '%');
                    $fq->orWhere('item_id', 'LIKE', '%' . $search . '%');
                    $fq->orWhere('item_barcode', 'LIKE', '%' . $search . '%');
                });
            });
            ...
            // ->get();
            // $result = \App\Item::where(['is_active' => '0', 'cid' => $cid])
            //     // ->toSql();    $q->where('sex', Auth::user()->sex)->orWhere('sex', 0);
            //     ->where(function ($q) {
            //         $search = $t;
    ...
    public function fetch_bill()
    {
        ...
        $total_amount = 0;
        $tdata = '';
        $bill_master_data = \App\BillMaster::select('*')->leftjoin('bil_addCustomer', 'bil_addCustomer.cust_id', '=', 'bil_AddBillMaster.cust_id')->where('bill_no', '=', $bill_no)->first();
